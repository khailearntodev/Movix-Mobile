...
export const PartyCard = ({ item, onPress }: PartyCardProps) => {
    // Determine status directly from the new flat backend map or fallback
    const isEnded = item.status === 'ended';
    const isLive = item.status === 'live';
    const isScheduled = item.status === 'scheduled';
    
    // Some Fallbacks if status is somehow magically missing but we have schedule_time (from older definitions)
    const displayScheduledAt = item.scheduledAt || item.schedule_time;

    return (
        <TouchableOpacity
            activeOpacity={0.9}
            onPress={onPress}
            className={clsx('mb-4 rounded-xl overflow-hidden bg-[#1F1F1F] border border-slate-800', isEnded && 'opacity-70 grayscale')}
        >
            <View className="h-48 w-full relative">
                ...
                <View className="absolute bottom-0 left-0 right-0 p-3 bg-black/60 backdrop-blur-sm">
                    <View className="flex-row items-center gap-2 mb-1">
                        ...
                        <Text className="text-slate-300 text-[10px] font-medium shadow-black">
                            {item.host}
                        </Text>
                    </View>
            ...
            {isScheduled && displayScheduledAt && (
                <View className="flex-row items-center justify-between p-3 bg-[#1a1a1a] border-t border-white/5">
                    <View className="flex-row items-center gap-1.5">
                        ...
                        <Text className="text-yellow-500 text-xs font-medium">
                            {new Date(displayScheduledAt).toLocaleString('vi-VN', { 
                                hour: '2-digit', 
                                minute: '2-digit', 
                                day: '2-digit', 
                                month: '2-digit' 
                    ...
                    <TouchableOpacity className="flex-row items-center gap-1.5" onPress={() => {}}>
